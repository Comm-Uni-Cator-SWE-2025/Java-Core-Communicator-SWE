...
/**
 * Packet sent by a new joinee to the server when joining a meeting.
 * Contains the email address of the joinee and their network coordinates.
 */
public class JoinPacket {
    /**
     * The email address of the user joining the meeting.
     */
    private final String email;

    /**
     * The network coordinates of the joinee.
     */
    private final ClientNode clientNode;

    /**
     * Constructs a new JoinPacket.
     *
     * @param email The email address of the user joining the meeting
     * @param clientNode The network coordinates of the user
     */
    public JoinPacket(final String email, final ClientNode clientNode) {
        if (email == null) {
    ...
    /**
     * Serializes the JoinPacket into a byte array.
     * Format:
     * - 1 byte: packet type (MeetingPacketType.JOIN.ordinal())
     * - 4 bytes: IP address (as int, network byte order)
     * - 2 bytes: port (as short)
     * - N bytes: email string (UTF-8, no length prefix since it's the last field)
     *
    ...
    public byte[] serialize() {
        try {
            ...
            buffer.put((byte) MeetingPacketType.JOIN.ordinal());
    ...
    /**
     * Deserializes a byte array into a JoinPacket.
     *
     * @param data The byte array to deserialize
     * @return The deserialized JoinPacket
     * @throws InvalidParameterException If the packet type is invalid or data is malformed
     */
    public static JoinPacket deserialize(final byte[] data) {
        if (data == null || data.length < 7) {
            throw new InvalidParameterException("Invalid data: too short for JoinPacket (need at least 7 bytes)");
        }

        final ByteBuffer buffer = ByteBuffer.wrap(data);

        final byte packetType = buffer.get();
        if (packetType != MeetingPacketType.JOIN.ordinal()) {
            throw new InvalidParameterException(
                "Invalid packet type: Expected " + MeetingPacketType.JOIN.ordinal() + " got: " + packetType);
        }
        ...
        return new JoinPacket(email, new ClientNode(host, port));
    }
}
